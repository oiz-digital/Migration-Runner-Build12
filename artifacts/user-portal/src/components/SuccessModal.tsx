 ...
 * Used for: spot orders, AI plan subscriptions, trading bot creation.
 */
...
export type SuccessPayload = OrderSuccess | PlanSuccess | BotSuccess;
...
function Row({
  ...
  return (
    ...
    >
      ...
      <span className={`text-sm font-bold font-mono ${accent ?? "text-foreground"}`}>
        {value}
      </span>
    </div>
  );
  ...
  if (payload.kind === "order") {
    ...
    rows = (
      <>
        <Row
          label="Side"
          value={<span style={{ color: accentColor }}>{isBuy ? "▲ BUY" : "▼ SELL"}</span>}
          delay={0}
        />
        ...
        <Row
          label={isBuy ? "Total Cost" : "You Receive"}
          value={totalFormatted}
          accent="text-amber-300"
          delay={3}
        />
        {payload.orderId && (
          <Row label="Order ID" value={`#${payload.orderId}`} accent="text-muted-foreground" delay={4} />
        )}
      </>
    );
    actions = (
      <div className="flex gap-2 mt-5">
        {onViewOrders && (
          <Button
            variant="outline"
            className="flex-1 border-white/10 gap-1.5 text-xs h-9"
  ...
  /* ── AI Plan ─────────────────────────────────────────────── */
    ...
    rows = (
      <>
        ...
        <Row
          label="Duration"
          value={payload.durationDays ? `${payload.durationDays} days` : "Unlimited ∞"}
          delay={3}
        />
        {payload.expectedProfit > 0 && (
          <Row
            label="Expected Profit"
            value={`+$${fmt(payload.expectedProfit, 2)} USDT`}
            accent="text-amber-300"
            delay={4}
          />
        )}
      ...
      >
        ...
        <div className="px-6 pt-7 pb-6">
          ...
          <div
            ...
            style={{ animation: "sm-row 0.3s 0.48s ease both", opacity: 0 }}
          >
            <h2
          ...
          <div
            ...
            style={{
              ...
              animation: "sm-row 0.3s 0.54s ease both",
              opacity: 0,
            }}
          >
            {rows}
